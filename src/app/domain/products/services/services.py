...
class ProductService(SQLSpecAsyncService[AsyncpgDriver]):
...
class StoreService(SQLSpecAsyncService[AsyncpgDriver]):
...
class OracleVectorSearchService:
    ...
    async def explain_search_plan(self, query: str) -> ExplainPlan:
        """Run EXPLAIN against the vector-search SQL and pull the plan."""
        embedding, _ = await self.vertex_ai_service.get_text_embedding(
            query, task_type="RETRIEVAL_QUERY", return_cache_status=True
        )
        # In Postgres, EXPLAIN returns the result directly
        rows = await self.product_service.driver.select(

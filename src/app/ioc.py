...
class LitestarPersistenceProvider(Provider):
    """Postgres config (APP) + per-request driver (REQUEST)."""
...
class IntegrationsProvider(Provider):
    ...
    @provide(scope=Scope.APP)
    def provide_adk_store(self, config: AsyncpgConfig) -> AsyncpgADKStore:
        return AsyncpgADKStore(config=config)

    @provide(scope=Scope.APP)
    def provide_session_service(self, store: AsyncpgADKStore) -> SQLSpecSessionService:

...
-- Using cosine distance for vector similarity (normalized vectors)
CREATE INDEX product_embedding_idx ON product USING hnsw (embedding vector_cosine_ops)
    WITH (m = 16, ef_construction = 64);

CREATE INDEX intent_exemplar_embedding_idx ON intent_exemplar USING hnsw (embedding vector_cosine_ops)
    WITH (m = 16, ef_construction = 64);

CREATE INDEX embedding_cache_embedding_idx ON embedding_cache USING hnsw (embedding vector_cosine_ops)
    WITH (m = 16, ef_construction = 64);

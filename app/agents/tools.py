...
def create_product_lookup_tool(product_service: ProductService):
    ...
    async def get_product_details(product_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            if len(product_id) == 36 and "-" in product_id:
...
def create_session_management_tool(chat_service: ChatService):
...
def create_conversation_history_tool(chat_service: ChatService):
...
def create_intent_classification_tool(intent_service: IntentService):
...
def create_metrics_recording_tool(metrics_service: MetricsService):
